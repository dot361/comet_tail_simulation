...
9. [Validation & sanity checks](#validation--sanity-checks)
...
The simulator is split across plain `<script>` files loaded in dependency order from `index.html`. There is no bundler.

| File | Purpose |
|---|---|
| `Constants.js` | All numeric constants: AU, GMsun, SCALE, particle caps, planet orbital elements, etc. |
| `Utils.js` | Stateless helpers: Julian Day formatting, coordinate transforms (ecliptic → equatorial, scene → RA/Dec), CSV download, colour utilities. |
| `Physics.js` | Core orbital mechanics: Kepler's equation solver, planet positions, universal-variable propagator (Stumpff functions, Lagrange f/g), PQW→IJK rotation. |
| `BetaCurve.js` | Interactive β distribution curve editor. Manages control points, draws the canvas, and exposes `sampleBetaFromCurve(u)`. Self-initialises at parse time. |
| `ParticlesGPU.js` | Factory (`setupRawParticles`) that builds the WebGPU compute + render pipeline. Returns a `{ seed, update, resize, clear, max }` handle used by the rest of the sim. |
| `Engine.js` | Babylon.js engine initialisation: WebGPU → WebGL fallback, compute capability detection, mode badge. Globals: `canvas`, `engine`, `scene`, `hasCompute`, `useCompute`. |
| `Camera.js` | ArcRotateCamera setup and all locking modes (J2000 fixed position, Earth-following). Functions: `setFocusOnComet`, `setViewAxis`, `lockCameraPositionToJ2000`, `lockCameraToEarth`. |
| `World.js` | AU reference grid, Babylon GUI layer, Sun mesh + glow, starfield point cloud. |
| `Planets.js` | Planet meshes, orbit lines, and labels for all eight planets plus Earth (tracked separately). |
| `Comet.js` | Comet orbital elements (globals `e`, `q`, `i`, `Omega`, `omega`, `t0`), `cometStateAtJD()` propagation, orbit drawing, preset activation, and `updateOrbitParameters()`. |
| `SyndyneSynchrone.js` | Synchrone and syndyne generation, position-angle computation, RA/Dec CSV export. Buttons are enabled only while the sim is paused. |
| `Particles.js` | CPU + GPU particle slot management: `createTailParticle()`, `generateBeta()`, `initParticles()`. Tracks `simSeconds`, `expiryByIndex`, and CPU mesh pool. |
| `UI.js` | Wires every control to its global: speed slider, vis-mode dropdown, orbit/particle inputs, pause button, keyboard shortcuts. |
| `RenderLoop.js` | `startRenderLoop()` — the per-frame loop: advances `simulationTimeJD`, computes activity Q, emits particles, updates CPU particle positions and colours, updates planet positions. |
| `main.js` | Entry point. Owns `simulationTimeJD`, `simulationSpeed`, `isPaused`. Calls all `initX()` functions in order, then `startRenderLoop()`. Exposes `setSimTime()` globally. |
